...
class XNLI(ClassificationTask):
    def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
        super(XNLI, self).__init__(
            config, "xnli", tokenizer, ["contradiction", "entailment", "neutral"]
        )
    ...
    def get_examples(self, split):
        ...
        if split not in ["train", "dev"]:
            raise RuntimeError(
                f"Expected 'train' or 'dev' for split, got '{split}' instead"
            )
        if split == "dev":
            split = "test"
        ds = datasets.load_dataset("xnli", "es", split=split)
        examples = []
...
class PAWSX(ClassificationTask):
    def __init__(self, config: configure_finetuning.FinetuningConfig, tokenizer):
        super(PAWSX, self).__init__(
            config, "pawsx", tokenizer, ["0", "1"]
        )
    ...
    def get_examples(self, split):
        ...
        if split == "dev":
            split = "test"
        ds = datasets.load_dataset("paws-x", "es", split=split)
        examples = []
        for i, row in enumerate(ds):
            examples.append(
                InputExample(
