...
def tokenize_and_align(tokenizer, words, cased=True):
...
class CoNLL2002POS(TaggingTask):
    """CoNLL2002 POS dataset"""

    def __init__(self, config, tokenizer):
        super(CoNLL2002POS, self).__init__(config, "conll2002pos", tokenizer, True)

    def _get_labeled_sentences(self, split):
        if split != "train" and split != "dev":
            raise RuntimeError(
                f"Expected split to be either 'train' or 'dev', found '{split}'!"
            )
        if split == "dev":
            split = "test"

        conll = datasets.load_dataset("conll2002", "es", split=split)
        pos_feature = conll.features["pos_tags"].feature
        conll_str = conll.map(
            lambda x: {
                "pos_tags_str": pos_feature.int2str(x["pos_tags"]),
            }
        )

        sentences = []
        for row in conll_str:
            sentences.append((row["tokens"], row["pos_tags_str"]))
            if self.config.debug and len(sentences) > 100:
                return sentences
    ...
    def _get_label_mapping(self, provided_split=None, provided_sentences=None):
        ...
        conll = datasets.load_dataset("conll2002", "es", split="test")
        pos_feature = conll.features["pos_tags"].feature
        label_mapping = {label: pos_feature.str2int(label) for label in pos_feature.names}
...
class CoNLL2002NER(TaggingTask):
    """CoNLL2002 POS dataset"""

    def __init__(self, config, tokenizer):
        super(CoNLL2002NER, self).__init__(config, "conll2002ner", tokenizer, True)
    ...
    def _get_labeled_sentences(self, split):
        ...
        conll = datasets.load_dataset("conll2002", "es", split=split)
        ner_feature = conll.features["ner_tags"].feature
        conll_str = conll.map(
            lambda x: {
                "ner_tags_str": ner_feature.int2str(x["ner_tags"]),
            }
        )

        sentences = []
        for row in conll_str:
            sentences.append((row["tokens"], row["ner_tags_str"]))
            if self.config.debug and len(sentences) > 100:
                return sentences
    ...
    def _get_label_mapping(self, provided_split=None, provided_sentences=None):
        ...
        conll = datasets.load_dataset("conll2002", "es", split="test")
        ner_feature = conll.features["ner_tags"].feature
        label_mapping = {label: ner_feature.str2int(label) for label in ner_feature.names}
    ...
    def get_scorer(self):
        return tagging_metrics.CoNLL2002NERScorer(self._get_label_mapping())
